...
const AUTO_GENERATED_ID = 'AutoGeneratedID'
type AOA = any[][];
@Component({
...
export class ImportLocationListComponent implements OnInit {
  ...
  generateIDs2() {
    // Get a list of levels we'll need to generate IDs for.
    const autoGenerateIdsForTheseLevels = this.locationList.locationsLevels.reduce((autoGenerateIdsForTheseLevels, level) => {
      return [
        ...autoGenerateIdsForTheseLevels,
        ...this.mappings[`${level}_id`] === 'AutoGeneratedID' ? [level] : []
      ]
    }, [])
    // Create a matrix of matrix[Level][Actual ID] = Determined ID. For levels that we generated IDs for, the "Actual ID",
    // is the value in the label column while the Determined ID is a UUID. For levels that we don't generate IDs for, the
    // Actual ID and the Determined ID are the same. 
    const determinedIdsByLevelAndActualId = this.locationList.locationsLevels.reduce((determinedIdsByLevelAndActualId, level) => {
      determinedIdsByLevelAndActualId[level] = this.parsedCSVData.reduce((idMaps, row) => {
        const key = autoGenerateIdsForTheseLevels.includes(level)
          ? row[`${level}_label`]
          : row[`${level}_id`]
        const value = autoGenerateIdsForTheseLevels.includes(level)
          ? uuidv4()
          : row[`${level}_id`]
        idMaps[key] = value
        return idMaps
      }, {})
      return determinedIdsByLevelAndActualId
    }, {})
    console.log(determinedIdsByLevelAndActualId)
    // Use matrix to map in determined IDs.
    this.parsedCSVData = this.parsedCSVData.map((row) => {
      for (const level of this.locationList.locationsLevels) {
        const actualId = autoGenerateIdsForTheseLevels.includes(level)
          ? row[`${level}_label`]
          : row[`${level}_id`]
        row[`${level}_id`] = determinedIdsByLevelAndActualId[level][actualId]
      }
      return row
    })
  }
