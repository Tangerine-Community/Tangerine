...
export class ImportLocationListComponent implements OnInit {
  ...
  validateMetadata() {
    const x = this.locationListLevels.map(level => {
      if (this.locationList.metadata[level]) {
        return this.locationList.metadata[level].map(metadataItem => {
          ...
          if (index > 0) {
            return ({ isValid: false, row: index + 2, column: this.locationsMetadata[level][metadataItem.variableName] });
      ...
      this.parsedCSVData.map((elem, index) => {
        levels.map((level, ind) => {
          if (this.mappings[`${levels[ind]}_id`] === this.autoGeneratedIDLabel) {
            const i = generatedIDs.findIndex(e => e.label.trim() === elem[this.mappings[`${levels[ind]}`]].trim()
              && e.level.trim() === level.trim());
            if (i >= 0) {
              ...
              generatedIDs = [...generatedIDs, { label: elem[this.mappings[`${levels[ind]}`]].trim(), level, [`${level}_id`]: id }];
            }
          }
        });
      });
