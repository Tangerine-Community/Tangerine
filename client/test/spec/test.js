(function() {
  ...
  tests = function(dbName) {
    ...
    describe('Foo test', function(done) {
      var foo;
      foo = false;
      ...
      return it("should pass", function() {
        return expect(foo).equals(true);
      });
    });
    return describe('Tangerine Tests', function(done) {
      var foo;
      foo = false;
      this.timeout(50000);
      before('Setup Tangerine and Pouch', function() {
        var pouchName;
        console.log("before, we are setting up Tangerine and Pouch");
      ...
      beforeEach('Setup done and timeout', function(done) {
        setTimeout(function() {
          foo = true;
          return done();
        }, 6000);
      });
      it('Populate pouch with Views and Assessments', function() {
        var db;
        console.log("Populating the pouchdb");
        db = Tangerine.db;
        ...
        return db.put({
          ...
          Utils.loadDevelopmentPacks(function(err) {
            return console.log("loadDevelopmentPacks done.");
          });
        });
      });
      ...
      it('Should display the Class Admin view', function() {
        var allKlasses;
        Tangerine.settings.set("context", "class");
        this.$fixture.empty().appendTo(this.$container);
                ...
                success: function() {
                  ...
                  return record.fetch({
                    ...
                    success: function(curriculaCollection) {
                      ...
                      Tangerine.currentView.teachers = teachers;
                      view.once("subRendered", function() {
                        var renderAdmin;
                        renderAdmin = function() {
                          console.log("view.$el.text():" + view.$el.text());
                          return expect(view.$el.text()).to.contain("classes");
                        };
                        return setTimeout(renderAdmin, 2000);
                      });
                      return view.render();
                    }
                  });
      ...
      return it('Should display the Class editor view', function() {
        var id, klass;
        Tangerine.settings.set("context", "class");
        this.$fixture.empty().appendTo(this.$container);
        id = "54DC5D9E-B8B1-B613-83F3-3683E15C6CE3";
        klass = new Klass({
          _id: id
        });
        return klass.fetch({
                ...
                return allStudents.fetch({
                  success: function(allStudents) {
                    ...
                    view.once("subRendered", function() {
                      ...
                      renderAdmin = function() {
                        console.log("view.$el.text():" + view.$el.text());
                        return expect(view.$el.text()).to.contain("tom");
                      };
                      return setTimeout(renderAdmin, 2000);
                    });
                    return view.render();
