...
#      dbs = [];
      ...
      before( 'Setup Tangerine and Pouch',() ->
        ...
        dbs = [dbName];
        #    // create db
        Tangerine.db = new PouchDB(pouchName, {adapter: 'memory'})
...
#      )
      ...
      it('Populate pouch with Views and Assessments', ()->
...
##       Save views
        ...
        ,(err, response) ->
          ...
          Utils.loadDevelopmentPacks((err) ->
            console.log("loadDevelopmentPacks done.")
          )
#            done()
          return
...
#        )
      ...
      it('Should return the expected curriculum', ()->
        id = "bccafd15-c49f-e29b-eae8-54bdb9a363e8"
        record = new Curriculum "_id" : id
        record.fetch({
          error:(err) ->
            console.log "Catch Error: " + JSON.stringify err
          success: (record) ->
#            console.log("record: " + JSON.stringify record)
            expect(record.get("name")).to.equal('Curr 1');
        })
      )

      it('Should display the Class Admin view', ()->
...
#                      klassCollection = new Klasses klassCollection.where("teacherId" : Tangerine.user.get("teacherId"))
                    ...
                    view.once("subRendered", () ->
                      renderAdmin = () ->
                        console.log("view.$el.text():" + view.$el.text())
                        expect(view.$el.text()).to.contain("classes")
      ...
      it('Should display the Class editor view', ()->
        Tangerine.settings.set("context", "class")
        this.$fixture.empty().appendTo(this.$container);
                ...
                allStudents.fetch
                  success: (allStudents) ->
                    ...
                    view.once("subRendered", () ->
                      renderAdmin = () ->
                        console.log("view.$el.text():" + view.$el.text())
                        expect(view.$el.text()).to.contain("tom")
                      setTimeout(renderAdmin, 2000)
                    )
                    view.render();
#                })
