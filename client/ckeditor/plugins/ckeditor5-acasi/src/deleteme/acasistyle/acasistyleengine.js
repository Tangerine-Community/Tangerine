...
import AcasiStyle from '../image/imageengine';
 ...
 */
export default class AcasiStyleEngine extends Plugin {
  /**
   * @inheritDoc
   */
  static get requires() {
  ...
  init() {
    ...
    editor.config.define( 'acasi.styles', [ 'imageStyleFull', 'imageStyleSide' ] );

    // Get configuration.
    const styles = this.imageStyles;

    // Allow imageStyle attribute in acasi.
    // We could call it 'style' but https://github.com/ckeditor/ckeditor5-engine/issues/559.
    schema.allow( { name: 'acasi', attributes: 'acasiStyle', inside: '$root' } );

    // Converters for acasiStyle attribute from model to view.
    const modelToViewConverter = modelToViewStyleAttribute( styles );
    editing.modelToView.on( 'addAttribute:acasiStyle:acasi', modelToViewConverter );
    data.modelToView.on( 'addAttribute:acasiStyle:acasi', modelToViewConverter );
    editing.modelToView.on( 'changeAttribute:acasiStyle:acasi', modelToViewConverter );
    data.modelToView.on( 'changeAttribute:acasiStyle:acasi', modelToViewConverter );
    editing.modelToView.on( 'removeAttribute:acasiStyle:acasi', modelToViewConverter );
    data.modelToView.on( 'removeAttribute:acasiStyle:acasi', modelToViewConverter );
  ...
  get acasiStyles() {
    ...
    const configuredStyles = editor.config.get( 'acasi.styles' );
   ...
   * {@link module:image/imagestyle/imagestyleengine~AcasiStyleEngine.defaultStyles} are available:
   *
   * * `'Full size image'`,
   * * `'Side image'`,
   * * `'Left aligned image'`,
 ...
 */
// AcasiStyleEngine.defaultStyles = {
...
function normalizeStyle( style ) {
  // const defaultStyles = AcasiStyleEngine.defaultStyles;
  const defaultIcons = AcasiStyleEngine.defaultIcons;
 ...
 *  * One of {@link module:image/imagestyle/imagestyleengine~AcasiStyleEngine.defaultIcons} to use a default icon provided by the plugin.
