...
def convert_response(resp_dict):
    ...
    if (type.lower() == "response"):
        ...
        try:
            ...
            #we will get the exceptionif the table doesn't exist yet, we need to create the table first
        ...
        try:
            ...
            #sometimes code stuck here, need to figure out why
...
#delete record call, this is not yet being used
...
def main_job():
    ...
    #login to Tangerine database
    tangerineClient = CouchDB(dbUserName, dbPassword, url=dbURL, connect=True)
    session = tangerineClient.session()
    tangerline_database = tangerineClient[dbName]
    ...
    config.set(thisTangerineSectionName, "LastSequence", last_change_seq)
    # Writing the configuration file to

    with open("data\\setting2.ini", 'w') as configfile:
...
#need to read in the number of databases we need to convert from Tangerine couchDB to MySQL, this can handle more thanone database at a time
totalDBs = config['TANGERINE-MYSQL-MAP']['total_database_count']
#find out the Scetion name for the Tangerine database configuration
tangerineSectionName = config['TANGERINE-MYSQL-MAP']['tangerine_section_name']
mySqlSectionSectionName = config['TANGERINE-MYSQL-MAP']['mysql_section_name']

for dbCounter in range(1, int(totalDBs)+1):
    thisTangerineSectionName = tangerineSectionName + str(dbCounter)
    thisMySqlSectionName = mySqlSectionSectionName + str(dbCounter)

    #get Tangerine database connection and connect to it
    dbURL = config[thisTangerineSectionName]['DatabaseURL']
    dbName = config[thisTangerineSectionName]['DatabaseName']
    dbUserName = config[thisTangerineSectionName]['DatabaseUserName']
    dbPassword = config[thisTangerineSectionName]['DatabasePassword']
    lastSequence = config[thisTangerineSectionName]['LastSequence']
    interval = config[thisTangerineSectionName]['run_interval']

    print('Tangerine Database URL:' +dbURL +' Database Name: ' + dbName + ' Username is: ' +dbUserName )
    #login
    # client = CouchDB(dbUserName, dbPassword, url=dbURL, connect=True)
    # session = client.session()
    # tangerline_database = client[dbName]
    # print(datetime.now().strftime("%m/%d/%Y, %H:%M:%S")+': Logged into Tangerine database' +dbName)

    #get MySQL database connection and connect to it
    mysqlHostName = config[thisMySqlSectionName]['HostName']
    mysqlDatabaseName = config[thisMySqlSectionName]['DatabaseName']
    mysqlUserName = config[thisMySqlSectionName]['UserName']
    mysqlPassword = config[thisMySqlSectionName]['Password']
