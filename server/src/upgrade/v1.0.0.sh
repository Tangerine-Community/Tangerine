...
curl http://$T_COUCHDB_USER_ADMIN_NAME:$T_COUCHDB_USER_ADMIN_PASSWORD@$T_COUCH_HOST:$T_COUCH_PORT/_all_dbs | jq '.' | grep 'group-' | sed -e 's/\"//g' | sed -e 's/,//' | while read database 
do
  curl -X POST -H 'Content-Type: application/json' -d @/tangerine-server/documents-for-new-groups/LocationList.json http://$T_COUCHDB_USER_ADMIN_NAME:$T_COUCHDB_USER_ADMIN_PASSWORD@$T_COUCH_HOST:$T_COUCH_PORT/$database
done
